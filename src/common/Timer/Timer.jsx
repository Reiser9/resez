...
const Timer = ({hours = 0, minutes = 0, seconds = 0}) => {
	...
	const tick = () => {
		...
		else if(minutesLeft === 0 && secondsLeft === 0) setTime([hoursLeft - 1, 59, 59]);
		else if(secondsLeft === 0) setTime([hours, minutesLeft - 1, 59]);
		else setTime([hoursLeft, minutesLeft, secondsLeft - 1]);

		if(minutesPass === 59 && secondsPass === 59) setTimePass([hoursPass + 1, 0, 0]);
		else if(secondsPass === 59) setTimePass([hoursPass, minutesPass + 1, 0]);
		else setTimePass([hoursPass, minutesPass, secondsPass + 1]);
	}
